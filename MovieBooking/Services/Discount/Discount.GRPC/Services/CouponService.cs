...
namespace Discount.GRPC.Services
{
    public class CouponService : CouponProtoService.CouponProtoServiceBase
    {
        ...
        {
            var coupon = await _repository.GetDiscount(request.MovieName);
            if(coupon == null)
            {
                throw new RpcException(new Status(StatusCode.NotFound, $"Discount for {request.MovieName} is not found"));
            }
            
            _logger.LogInformation("Discount for movie: {movieName}, amount: {amount}",
                coupon.MovieName, coupon.Amount);
        ...
        public override async Task<GetRandomDiscountsResponse> GetRandomDiscounts(GetRandomDiscountsRequest request, ServerCallContext context)
        {
            ...
            _logger.LogInformation("{numberOfDiscounts} random coupons with total value: {amount}",
